 ...
 */
 ...
 // linear O(n) as the num input increases the # of operations increase proportionally
...
// linear O(n) directly proportional operation to array size
...
// linear O(n) proportional to how long the string is
...
// linear O(n) proportional to how large the triangle is
function triangle(n) {
  var tot = 0;
  for (var i = 1; i <= n; ++i) {
	    tot += n;
...
// O(logn) the input number halfs each time therefore the number of operations are much less than the size of the input #
...
// O(n) linear, directly proportional between the number input and amount of operations
