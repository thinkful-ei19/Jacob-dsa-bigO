...
// increment outside of recursive scope, log out in the base case


// O(n) linear proportion of sheep to operations
...
reverseString('1234567891239841');
...
// linear O(n) directly proportional operations to n input
function triangle(n) {
  if (n < 2) {
    return n;
  }
...
let inc = 0;
// O(n) linear , proportional to the number of seperators
...
// O(logn) as the number input increases, the number of operations grows slower
...
//O(n^2), has to split into two recursive stacks which loop through to solve n
...
// O(n!) as the word increases in size it has to permutate through all of the different combinations,
// using a for loop within each recursive call, therefore the number of operations drastically increases
...
let animalHierarchyCount = 0;
// ==============================
function traverse(animalHierarchy, parent) {
    animalHierarchyCount++;
  let node = {};
...
console.log('AMINAL',animalHierarchyCount);
/*=================================================================================*/
/*
...
//linear O(n) # of people proportionate to operations
let organization = {
  "Zuckerberg": {		
    "Schroepfer": {
      "Bosworth": {
